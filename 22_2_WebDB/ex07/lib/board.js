...
module.exports = {
    list : function(request,response){
        db.query(`SELECT count(*) as total FROM board`,function(error, nums){
            ...
            db.query(`SELECT * FROM board ORDER BY created desc, id LIMIT ? OFFSET ?`, 
            [numPerPage, offs],function(error, boards) {
    ...
    create : function(request,response){
            db.query(`SELECT *  FROM person WHERE loginid = ? `,[request.session.loginid],
    ...
    create_process : function(request,response){
        ...
        request.on('end', function() {
            ...
            db.query(`
                INSERT INTO board (loginid, name, date, content,title, created)
                    VALUES(?, ?, ?, ?, ?, NOW())`,
                [post.id, post.nmh, dateOfEightDigit(),post.content, post.title], 
    ...
    update : function(request, response) {
        var bNum = request.params.bNum;
        var pNum = request.params.pNum;
        db.query(`SELECT *  FROM person WHERE loginid = ? `,[request.session.loginid],function(error, person){
            db.query(`SELECT * FROM board WHERE id= ?`,[bNum],function(error,board){
                if(error) {
                    throw error;
                }
